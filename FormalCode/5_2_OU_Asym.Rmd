...
```{r}
#
#save.image(file = "Rda/asym_tp1_v1.RData") # tree test hitree=2
...
#save.image(file = "Rda/tree_p_asym_0424.RData")
```

here
```{r}
#load("D:/DataScience/LiuCourses/masterThesis/TheFormalCode/Rdata/TX0_symH_01.RData")
#source("tool_funs.R")
```
...
# case2some
#egv_2st_H<-0.8
#egv_1st_H<-c(seq(from = egv_2st_H, to = 6,by=0.5)[-c(1)])
#egv_H_lst<-lapply(egv_1st_H, function(x)c(x,egv_2st_H)) 
...
#c_sel<-c(1,2,21,22,24,25,35,46)  #0419
...
1.2   careful observing the trend---------------------------------------------
```{r}
#t_dim=1
test_egv_H_llk<-function(t_dim){
  egv_H_arr[,,t_dim]
...
1.2   careful observing the trend on the selected data sets,
```{r}
#t_dim=1
test_egv_H_llk<-function(t_dim){
     print(paste0(H_i,":"))
...
qs_rt_lst
 ...
 qs_rt_lst[[4]] %>% layout(title =  TeX("\\textbf{Likelihood vs Experiment Eigenvalues in the First Dimension of } \\mathbf H"),
         xaxis = list(title = TeX("\\textbf{Angle of Rotation Matrix}")
...
#c_sel_al1<-c_sel_al
res_tb1<-list()  #c(2,4,5,6,7,8,10,13,16)
res_tb2<-list() #c(9,11,14,17,18)
...
egv_H_lst[c(2,5,9,11,14,18)]   # 


# c(2,4,5,6,7,10,11,13,15)

egv_H_lst[c(2,4,5,6,7,10,11,13,15)]
...
#res_tb2_td_56<-list()
for(ix in c(1:length(c_sel_al1)) ){
  res_tb1_td_7[[ix]]<-table((tr56_1_lik_lst[c_sel_al1[ix]][[1]][c(2:ll)]- tr56_1_lik_lst[c_sel_al1[ix]][[1]][c(1:(ll-1))])>0)
 # res_tb2_td_56[[ix]]<-table((tr56_2_lik_lst[c_sel_al1[ix]][[1]][c(2:ll)]- tr56_2_lik_lst[c_sel_al1[ix]][[1]][c(1:(ll-1))])>0)
